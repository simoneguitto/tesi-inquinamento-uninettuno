...
np.random.seed(42)
if num_palazzi > 0:
    # Palazzo critico posizionato sulla traiettoria del vento
    edifici[dist_primo_palazzo:dist_primo_palazzo+4, 23:27] = 1
    # Altri palazzi distribuiti casualmente nel dominio
...
# --- MOTORE DI CALCOLO ADR (Advezione-Diffusione-Reazione) ---
if st.sidebar.button("ESEGUI ANALISI METEOROLOGICA"):
    C = np.zeros((N, N))
    mappa_box = st.empty()
    testo_box = st.empty()
    sx, sy = 5, 25 # Sorgente del gas
    ...
    for t in range(160):
        ...
        for i in range(1, N-1):
            for j in range(1, N-1):
                ...
                # Discretizzazione equazione differenziale
        ...
        # Condizioni al contorno (ostacoli) e stabilità numerica
        ...
        if t % 15 == 0:
            ...
            # Rendering del grafico 3D con Plotly
            fig = go.Figure(data=[
            ...
            if picco > 0.12:
                testo_box.error(f"⚠️ SOGLIA CRITICA: {picco:.4f} ppm")
            else:
                testo_box.success(f"✅ STATO SICURO: {picco:.4f} ppm")

    st.info("Simulazione completata. Studio condotto secondo i modelli di trasporto atmosferico.")
